...
static std::string hwcloud_proxy_host;
static int hwcloud_proxy_port;
static std::string hwcloud_proxy_user;
static std::string hwcloud_proxy_passwd;

// List for all hwcloud commands
static SimpleList<HwcloudGahpCommand*> hwcloud_gahp_commands;

void set_hwcloud_proxy_server(const char* url)
{
	if( !url ) {
		return;
	}
	...
	if( !strncasecmp("http://", url, strlen("http://"))) {
		hwcloud_proxy_host = url +  strlen("http://");
		hwcloud_proxy_port = 80;
	}else if( !strncasecmp("https://", url, strlen("https://")) ) { 
		hwcloud_proxy_host = url +  strlen("https://");
		hwcloud_proxy_port = 443;
	}else {
		hwcloud_proxy_host = url;
		hwcloud_proxy_port = 80;
	}
	
	/* sateesh added code to even handle proxy username and password */
	/* This code cannot handle passwords containing @ ? */
	/* Exact format of HWCLOUD_HTTP_PROXY is -- http://userid:password@host:port */
    size_t pos = hwcloud_proxy_host.find('@');
	if( std::string::npos != pos ) {
	  hwcloud_proxy_user = hwcloud_proxy_host.substr(0, pos);
	  
	  hwcloud_proxy_host = hwcloud_proxy_host.substr(pos + 1,
		  hwcloud_proxy_host.length());

      pos = hwcloud_proxy_user.find(':');
	  if( std::string::npos != pos ) {
		hwcloud_proxy_passwd = hwcloud_proxy_user.substr(pos + 1,
			hwcloud_proxy_user.length());
		hwcloud_proxy_user = hwcloud_proxy_user.substr(0, pos);
	  }
	}

    pos = hwcloud_proxy_host.find(':');
	if( std::string::npos != pos ) {
		int port =
			atoi(hwcloud_proxy_host.substr(pos + 1,
										  hwcloud_proxy_host.length()).c_str());

		if( port > 0 ) {
			hwcloud_proxy_port = port;
		}

		hwcloud_proxy_host = hwcloud_proxy_host.substr(0, pos);
	}
         
	dprintf(D_ALWAYS, "Using proxy server, host=%s, port=%d user=%s\n", 
		hwcloud_proxy_host.c_str(), hwcloud_proxy_port,
		hwcloud_proxy_user.c_str());
}

bool get_hwcloud_proxy_server(const char* &host_name, int& port, const char* &user_name, const char* &passwd )
{
	if( hwcloud_proxy_host.empty() == false ) {
		host_name = hwcloud_proxy_host.c_str();
		port = hwcloud_proxy_port;
		user_name = hwcloud_proxy_user.c_str();
		passwd = hwcloud_proxy_passwd.c_str();
		return true;
	}
...
HwcloudGahpCommand::HwcloudGahpCommand(const char* cmd, ioCheckfn iofunc, workerfn workerfunc)
{
	command = cmd;
...
void
registerHwcloudGahpCommand(const char* command, ioCheckfn iofunc, workerfn workerfunc)
{
	if( !command ) {
		return;
	}

	HwcloudGahpCommand* newcommand = new HwcloudGahpCommand(command, iofunc, workerfunc);
	ASSERT(newcommand);

	hwcloud_gahp_commands.Append(newcommand);
}

int
numofHwcloudCommands(void)
{
	return hwcloud_gahp_commands.Number();
}

int 
allHwcloudCommands(StringList &output)
{
	HwcloudGahpCommand *one_cmd = NULL;

	hwcloud_gahp_commands.Rewind();
	while( hwcloud_gahp_commands.Next(one_cmd) ) {
...
create_success_result( int req_id, StringList *result_list)
{
	...
	tmp_result[0] = HWCLOUD_COMMAND_SUCCESS_OUTPUT;
...
create_failure_result( int req_id, const char *err_msg, const char* err_code)
{
	...
	tmp_result[0] = HWCLOUD_COMMAND_ERROR_OUTPUT;
