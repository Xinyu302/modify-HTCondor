...
#ifndef __HWCLOUDGAHP_COMMON_H__
#define __HWCLOUDGAHP_COMMON_H__
...
#define HWCLOUD_SCRIPT_INTERPRETER "perl"
#define HWCLOUD_SCRIPT_NAME "condor_hwcloud.pl"

#define HWCLOUD_COMMAND_SUCCESS_OUTPUT	"0"
#define HWCLOUD_COMMAND_ERROR_OUTPUT		"1"

#define HWCLOUD_HTTP_PROXY   "HWCLOUD_HTTP_PROXY"

typedef bool (*ioCheckfn)(char **argv, int argc);
typedef bool (*workerfn)(char **argv, int argc, std::string &output_string);

class HwcloudGahpCommand
{
	public:
		HwcloudGahpCommand(const char*, ioCheckfn, workerfn);
		std::string command;
...
int numofHwcloudCommands(void);
int allHwcloudCommands(StringList &output);
