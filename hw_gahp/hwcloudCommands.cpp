...
// except for by HwcloudQuery::SendRequest().
//
std::string hwcloudURLEncode( const std::string & input )
{
    /*
     * See http://docs.hwcloudwebservices.com/AWSEC2/2010-11-15/DeveloperGuide/using-query-api.html
     *
     *
...
class Throttle {
	...
	public:
		// Managed by HwcloudRequest::SendRequest() because the param system
...
bool HwcloudRequest::SendRequest() {
...
std::string HwcloudRequest::canonicalizeQueryString() {
    ...
    for( auto i = query_parameters.begin(); i != query_parameters.end(); ++i ) {
        ...
        // and then compose the string out of that, in case hwcloudURLEncode()
        // changes the sort order, but we don't specify parameters like that.

        // Step 1B: Encode the parameter names and values.
        std::string name = hwcloudURLEncode( i->first );
        std::string value = hwcloudURLEncode( i->second );
...
std::string pathEncode( const std::string & original ) {
	...
	while( offset < length ) {
		...
		encoded += hwcloudURLEncode( segment );
...
bool HwcloudRequest::createV4Signature(	const std::string & payload,
	...
	HwcloudRequest::AttributeValueMap transformedHeaders;
...
bool HwcloudRequest::sendV4Request( const std::string & payload, bool sendContentSHA ) {
...
bool HwcloudRequest::sendV2Request() {
    //
     ...
     * the signature.  See
     *
     * http://docs.hwcloudwebservices.com/AWSEC2/2010-11-15/DeveloperGuide/using-query-api.html
     *
     */
    ...
    canonicalQueryString += "&Signature=" + hwcloudURLEncode( signatureInBase64 );
    std::string postURI;
    if( protocol == "x509" ) {
...
bool HwcloudRequest::SendURIRequest() {
    httpVerb = "GET";
    std::string noPayloadAllowed;
    return sendV4Request( noPayloadAllowed );
}

bool HwcloudRequest::SendJSONRequest( const std::string & payload ) {
...
bool HwcloudRequest::SendS3Request( const std::string & payload ) {
...
bool HwcloudRequest::sendPreparedRequest(
...
*/
    ...
    hwcloud_gahp_release_big_mutex();
...
bool HwcloudVMStart::SendRequest() {
    bool result = HwcloudRequest::SendRequest();
    if ( result ) {
...
bool HwcloudVMStart::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_START" ) == 0 );

    // Uses the Query API function 'RunInstances', as documented at
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/ApiReference-query-RunInstances.html
    ...
    HwcloudVMStart vmStartRequest = HwcloudVMStart( requestID, argv[0] );
...
HwcloudVMStartSpot::~HwcloudVMStartSpot() { }
...
bool HwcloudVMStartSpot::SendRequest() {
    bool result = HwcloudRequest::SendRequest();
    if ( result ) {
...
bool HwcloudVMStartSpot::workerFunction( char ** argv, int argc, std::string & result_string ) {
    assert( strcasecmp( argv[0], "EC2_VM_START_SPOT" ) == 0 );

    // Uses the Query AP function 'RequestSpotInstances', as documented at
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/ApiReference-query-RequestSpotInstances.html
    ...
    HwcloudVMStartSpot vmSpotRequest = HwcloudVMStartSpot( requestID, argv[0] );
    ...
    return true;
} // end HwcloudVMStartSpot::workerFunction()

// ---------------------------------------------------------------------------

HwcloudVMStop::~HwcloudVMStop() { }

// Expecting:EC2_VM_STOP <req_id> <serviceurl> <accesskeyfile> <secretkeyfile> <instance-id>
bool HwcloudVMStop::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_STOP" ) == 0 );

    // Uses the Query API function 'TerminateInstances', as documented at
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/ApiReference-query-TerminateInstances.html
    ...
    // Fill in required attributes & parameters.
    HwcloudVMStop terminationRequest = HwcloudVMStop( requestID, argv[0] );
    ...
    if( ! terminationRequest.SendRequest() ) {
        ...
        // HwcloudVMStop::SendRequest() could parse its resultString
...
HwcloudVMStopSpot::~HwcloudVMStopSpot() { }

bool HwcloudVMStopSpot::workerFunction( char ** argv, int argc, std::string & result_string ) {
    assert( strcasecmp( argv[0], "EC2_VM_STOP_SPOT" ) == 0 );

    // Uses the Query API function 'CancelSpotInstanceRequests', documented at
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/ApiReference-query-CancelSpotInstanceRequests.html
    ...
    HwcloudVMStopSpot terminationRequest = HwcloudVMStopSpot( requestID, argv[0] );
    terminationRequest.serviceURL = argv[2];
    terminationRequest.accessKeyFile = argv[3];
    terminationRequest.secretKeyFile = argv[4];
    ...
    return true;
} // end HwcloudVMStop::workerFunction()

// ---------------------------------------------------------------------------

HwcloudVMStatus::~HwcloudVMStatus() { }

// Expecting:EC2_VM_STATUS <req_id> <serviceurl> <accesskeyfile> <secretkeyfile> <instance-id>
bool HwcloudVMStatus::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_STATUS" ) == 0 );

    // Uses the Query API function 'DescribeInstances', as documented at
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/ApiReference-query-DescribeInstances.html
    ...
    // Fill in required attributes & parameters.
    HwcloudVMStatus sRequest = HwcloudVMStatus( requestID, argv[0] );
    ...
    if( ! sRequest.SendRequest() ) {
        ...
        if( sRequest.results.size() == 0 ) {
            ...
            for( unsigned i = 0; i < sRequest.results.size(); ++i ) {
                HwcloudStatusResult asr = sRequest.results[i];
                ...
                if( strcasecmp( asr.status.c_str(), HWCLOUD_STATUS_RUNNING ) == 0 ) {
...
HwcloudVMStatusSpot::~HwcloudVMStatusSpot() { }
...
struct vmStatusSpotUD_t {
    ...
    HwcloudStatusSpotResult * currentResult;

    std::vector< HwcloudStatusSpotResult > & results;

    vmStatusSpotUD_t( std::vector< HwcloudStatusSpotResult > & assrList ) :
        inItem( 0 ),
        inStatus( false ),
...
void vmStatusSpotESH( void * vUserData, const XML_Char * name, const XML_Char ** ) {
    ...
    if( strcasecmp( ignoringNameSpace( name ), "item" ) == 0 ) {
        if( vsud->inItem == 0 ) {
            vsud->currentResult = new HwcloudStatusSpotResult();
...
bool HwcloudVMStatusSpot::SendRequest() {
    bool result = HwcloudRequest::SendRequest();
    if( result ) {
    ...
    return result;
} // end HwcloudVMStatusSpot::SendRequest()

bool HwcloudVMStatusSpot::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_STATUS_SPOT" ) == 0 );

    // Uses the Query API function 'DescribeSpotInstanceRequests', as documented at
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/ApiReference-query-DescribeSpotInstanceRequests.html
    ...
    HwcloudVMStatusSpot statusRequest = HwcloudVMStatusSpot( requestID, argv[0] );
    ...
    if( ! statusRequest.SendRequest() ) {
        ...
        if( statusRequest.spotResults.size() == 0 ) {
            ...
            for( unsigned i = 0; i < statusRequest.spotResults.size(); ++i ) {
                HwcloudStatusSpotResult & assr = statusRequest.spotResults[i];
    ...
    return true;
} // end HwcloudVmStatusSpot::workerFunction()

// ---------------------------------------------------------------------------

HwcloudVMStatusAllSpot::~HwcloudVMStatusAllSpot( ) { }

bool HwcloudVMStatusAllSpot::workerFunction(char **argv, int argc, std::string &result_string) {
    ...
    HwcloudVMStatusAllSpot statusRequest = HwcloudVMStatusAllSpot( requestID, argv[0] );
    statusRequest.serviceURL = argv[2];
    statusRequest.accessKeyFile = argv[3];
    statusRequest.secretKeyFile = argv[4];
    statusRequest.query_parameters[ "Action" ] = "DescribeSpotInstanceRequests";
    ...
    return true;
} // end HwcloudVmStatusAllSpot::workerFunction()

// ---------------------------------------------------------------------------

HwcloudVMStatusAll::~HwcloudVMStatusAll() { }
...
struct vmStatusUD_t {
    ...
    HwcloudStatusResult * currentResult;
    ...
    vmStatusUD_t( std::vector< HwcloudStatusResult > & asrList ) :
...
void vmStatusESH( void * vUserData, const XML_Char * name, const XML_Char ** ) {
    ...
    if( ! vsud->inInstance ) {
        if( strcasecmp( ignoringNameSpace( name ), "item" ) == 0 ) {
            vsud->currentResult = new HwcloudStatusResult();
            assert( vsud->currentResult != NULL );
...
bool HwcloudVMStatusAll::SendRequest() {
    bool result = HwcloudRequest::SendRequest();
    if( result ) {
...
bool HwcloudVMStatusAll::workerFunction(char **argv, int argc, std::string &result_string) {
    ...
    // Fill in required attributes & parameters.
    HwcloudVMStatusAll saRequest = HwcloudVMStatusAll( requestID, argv[0] );
    ...
    if( ! saRequest.SendRequest() ) {
        ...
        if( saRequest.results.size() == 0 ) {
            ...
            for( unsigned i = 0; i < saRequest.results.size(); ++i ) {
                HwcloudStatusResult & asr = saRequest.results[i];
                resultList.append( asr.instance_id.c_str() );
                resultList.append( asr.status.c_str() );
...
HwcloudVMCreateKeypair::~HwcloudVMCreateKeypair() { }
...
bool HwcloudVMCreateKeypair::SendRequest() {
    bool result = HwcloudRequest::SendRequest();
    if( result ) {
        privateKeyUD pkud;
        XML_Parser xp = XML_ParserCreate( NULL );
...
bool HwcloudVMCreateKeypair::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_CREATE_KEYPAIR" ) == 0 );

    // Uses the Query API function 'CreateKeyPair', as documented at
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/ApiReference-query-CreateKeyPair.html
    ...
    // Fill in required attributes & parameters.
    HwcloudVMCreateKeypair ckpRequest = HwcloudVMCreateKeypair( requestID, argv[0] );
...
HwcloudVMDestroyKeypair::~HwcloudVMDestroyKeypair() { }

// Expecting:EC2_VM_DESTROY_KEYPAIR <req_id> <serviceurl> <accesskeyfile> <secretkeyfile> <keyname>
bool HwcloudVMDestroyKeypair::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_DESTROY_KEYPAIR" ) == 0 );

    // Uses the Query API function 'DeleteKeyPair', as documented at
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/ApiReference-query-DeleteKeyPair.html
    ...
    // Fill in required attributes & parameters.
    HwcloudVMDestroyKeypair dkpRequest = HwcloudVMDestroyKeypair( requestID, argv[0] );
...
HwcloudAssociateAddress::~HwcloudAssociateAddress() { }

// Expecting:EC2_VM_ASSOCIATE_ADDRESS <req_id> <serviceurl> <accesskeyfile> <secretkeyfile> <instance-id> <elastic-ip>
bool HwcloudAssociateAddress::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcasecmp( argv[0], "EC2_VM_ASSOCIATE_ADDRESS" ) == 0 );

    // Uses the Query API function 'AssociateAddress', as documented at
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/index.html?ApiReference-query-AssociateAddress.html
    ...
    // Fill in required attributes & parameters.
    HwcloudAssociateAddress asRequest = HwcloudAssociateAddress( requestID, argv[0] );
...
bool
HwcloudCreateTags::workerFunction(char **argv,
                                 int argc,
                                 std::string &result_string)
{
    ...
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/index.html?ApiReference-query-CreateTags.html
    ...
    // Fill in required attributes & parameters.
    HwcloudCreateTags asRequest = HwcloudCreateTags( requestID, argv[0] );
    asRequest.serviceURL = argv[2];
    asRequest.accessKeyFile = argv[3];
    asRequest.secretKeyFile = argv[4];
...
HwcloudAttachVolume::~HwcloudAttachVolume() { }

bool HwcloudAttachVolume::workerFunction(char **argv, int argc, std::string &result_string)
{
    ...
    // Fill in required attributes & parameters.
    HwcloudAttachVolume asRequest = HwcloudAttachVolume( requestID, argv[0] );
    asRequest.serviceURL = argv[2];
    asRequest.accessKeyFile = argv[3];
    asRequest.secretKeyFile = argv[4];
...
// The types detected are Hwcloud, OpenStack, Nimbus, and Eucalyptus.
// If the server's response doesn't match any of these types, then
// "Unknown" is returned.
// The following checks are performed:
// * Hwcloud's header includes "Server: HwcloudEC2"
...
// * Hwcloud and Nimbus's <?xml?> tag includes version="1.0" and
...
HwcloudVMServerType::~HwcloudVMServerType() { }

bool HwcloudVMServerType::SendRequest() {
    bool result = HwcloudRequest::SendRequest();
    if( result ) {
        serverType = "Unknown";
        ...
        if ( header.find( "Server: HwcloudEC2" ) != std::string::npos ) {
            server_hwcloud = true;
        }
        ...
        if ( server_hwcloud && !server_jetty && xml_tag &&
             xml_encoding && request_id && !euca_tag ) {
            serverType = "Hwcloud";
        } else if ( !server_hwcloud && !server_jetty && xml_tag &&
                    !xml_encoding && request_id && !euca_tag ) {
...
bool HwcloudVMServerType::workerFunction(char **argv, int argc, std::string &result_string) {
    assert( strcmp( argv[0], "EC2_VM_SERVER_TYPE" ) == 0 );

    // Uses the Query API function 'DescribeKeyPairs', as documented at
    // http://docs.hwcloudwebservices.com/AWSEC2/latest/APIReference/ApiReference-query-DescribeKeyPairs.html
    ...
    // Fill in required attributes & parameters.
    HwcloudVMServerType serverTypeRequest = HwcloudVMServerType( requestID, argv[0] );
...
HwcloudBulkStart::~HwcloudBulkStart() { }
...
bool HwcloudBulkStart::SendRequest() {
	bool result = HwcloudRequest::SendRequest();
	if( result ) {
...
void HwcloudBulkStart::setLaunchSpecificationAttribute(
...
bool HwcloudBulkStart::workerFunction( char ** argv, int argc, std::string & result_string ) {
	...
	// Fill in required attributes.
	HwcloudBulkStart request = HwcloudBulkStart( requestID, argv[0] );
...
HwcloudPutRule::~HwcloudPutRule() { }

bool HwcloudPutRule::SendJSONRequest( const std::string & payload ) {
	bool result = HwcloudRequest::SendJSONRequest( payload );
	if( result ) {
...
bool HwcloudPutRule::workerFunction( char ** argv, int argc, std::string & result_string ) {
	assert( strcasecmp( argv[0], HWCLOUD_COMMAND_PUT_RULE ) == 0 );
	int requestID;
	get_int( argv[1], & requestID );
	dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
		requestID, argv[0] );
	...
	// Fill in required attributes.
	HwcloudPutRule request = HwcloudPutRule( requestID, argv[0] );
	request.serviceURL = argv[2];
	request.accessKeyFile = argv[3];
	request.secretKeyFile = argv[4];
...
HwcloudPutTargets::~HwcloudPutTargets() { }

bool HwcloudPutTargets::SendJSONRequest( const std::string & payload ) {
	bool result = HwcloudRequest::SendJSONRequest( payload );
	if( result ) {
		// We could use a dedicated JSON parser, but its API is annoying,
		// and the ClassAd converter is good enough.
...
bool HwcloudPutTargets::workerFunction( char ** argv, int argc, std::string & result_string ) {
	assert( strcasecmp( argv[0], HWCLOUD_COMMAND_PUT_TARGETS ) == 0 );
	int requestID;
	get_int( argv[1], & requestID );
	dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
		requestID, argv[0] );
	...
	// Fill in required attributes.
	HwcloudPutTargets request = HwcloudPutTargets( requestID, argv[0] );
	request.serviceURL = argv[2];
	request.accessKeyFile = argv[3];
	request.secretKeyFile = argv[4];
...
HwcloudRemoveTargets::~HwcloudRemoveTargets() { }

bool HwcloudRemoveTargets::SendJSONRequest( const std::string & payload ) {
	bool result = HwcloudRequest::SendJSONRequest( payload );
	if( result ) {
		ClassAd reply;
		classad::ClassAdJsonParser cajp;
...
bool HwcloudRemoveTargets::workerFunction( char ** argv, int argc, std::string & result_string ) {
	assert( strcasecmp( argv[0], HWCLOUD_COMMAND_REMOVE_TARGETS ) == 0 );
	int requestID;
	get_int( argv[1], & requestID );
	dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
		requestID, argv[0] );
	...
	// Fill in required attributes.
	HwcloudRemoveTargets request = HwcloudRemoveTargets( requestID, argv[0] );
	request.serviceURL = argv[2];
	request.accessKeyFile = argv[3];
	request.secretKeyFile = argv[4];
...
HwcloudDeleteRule::~HwcloudDeleteRule() { }

bool HwcloudDeleteRule::SendJSONRequest( const std::string & payload ) {
	bool result = HwcloudRequest::SendJSONRequest( payload );
	// Succesful results are defined to be empty.
	return result;
}

bool HwcloudDeleteRule::workerFunction( char ** argv, int argc, std::string & result_string ) {
	assert( strcasecmp( argv[0], HWCLOUD_COMMAND_DELETE_RULE ) == 0 );
	int requestID;
	get_int( argv[1], & requestID );
	dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
		requestID, argv[0] );
	...
	// Fill in required attributes.
	HwcloudDeleteRule request = HwcloudDeleteRule( requestID, argv[0] );
	request.serviceURL = argv[2];
	request.accessKeyFile = argv[3];
	request.secretKeyFile = argv[4];
...
HwcloudBulkStop::~HwcloudBulkStop() { }
...
bool HwcloudBulkStop::SendRequest() {
	bool result = HwcloudRequest::SendRequest();
	if( result ) {
...
bool HwcloudBulkStop::workerFunction( char ** argv, int argc, std::string & result_string ) {
	...
	// Fill in required attributes.
	HwcloudBulkStop request = HwcloudBulkStop( requestID, argv[0] );
	request.serviceURL = argv[2];
	request.accessKeyFile = argv[3];
	request.secretKeyFile = argv[4];
	...
	// See comment in HwcloudBulkStart::workerFunction().
	request.query_parameters[ "Version" ] = "2016-04-01";
...
HwcloudGetFunction::~HwcloudGetFunction() { }

bool HwcloudGetFunction::SendURIRequest() {
	bool result = HwcloudRequest::SendURIRequest();
	if( result ) {
		ClassAd reply;
		classad::ClassAdJsonParser cajp;
		if(! cajp.ParseClassAd( this->resultString, reply, true )) {
...
bool HwcloudGetFunction::workerFunction( char ** argv, int argc, std::string & result_string ) {
	assert( strcasecmp( argv[0], HWCLOUD_COMMAND_GET_FUNCTION ) == 0 );
	int requestID;
	get_int( argv[1], & requestID );
	dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
		requestID, argv[0] );
	...
	// Fill in required attributes.
	HwcloudGetFunction request = HwcloudGetFunction( requestID, argv[0] );
	request.accessKeyFile = argv[3];
	request.secretKeyFile = argv[4];
	...
	formatstr( request.serviceURL, "%s%s2015-03-31/functions/%s", argv[2],
		separator, hwcloudURLEncode( argv[5] ).c_str() );
...
HwcloudS3Upload::~HwcloudS3Upload() { }

bool HwcloudS3Upload::SendRequest() {
	httpVerb = "PUT";
	std::string payload;
...
bool HwcloudS3Upload::workerFunction(char **argv, int argc, std::string &result_string) {
	...
	// Fill in required attributes & parameters.
	HwcloudS3Upload uploadRequest = HwcloudS3Upload( requestID, argv[0] );
...
HwcloudCreateStack::~HwcloudCreateStack() { }

bool HwcloudCreateStack::SendRequest() {
	bool result = HwcloudRequest::SendRequest();
	if( result ) {
		Regex r; int errCode = 0; const char * errString = 0;
...
bool HwcloudCreateStack::workerFunction(char **argv, int argc, std::string &result_string) {
	...
	// Fill in required attributes & parameters.
	HwcloudCreateStack csRequest = HwcloudCreateStack( requestID, argv[0] );
...
HwcloudDescribeStacks::~HwcloudDescribeStacks() { }

bool HwcloudDescribeStacks::SendRequest() {
	bool result = HwcloudRequest::SendRequest();
	if( result ) {
...
//				<serviceurl> <accesskeyfile> <secretkeyfile>
//				<stackName>
bool HwcloudDescribeStacks::workerFunction(char **argv, int argc, std::string &result_string) {
	...
	// Fill in required attributes & parameters.
	HwcloudDescribeStacks dsRequest = HwcloudDescribeStacks( requestID, argv[0] );
...
HwcloudCallFunction::~HwcloudCallFunction() { }

bool HwcloudCallFunction::SendJSONRequest( const std::string & payload ) {
	bool result = HwcloudRequest::SendJSONRequest( payload );
	return result;
}
...
bool HwcloudCallFunction::workerFunction( char ** argv, int argc, std::string & result_string ) {
	assert( strcasecmp( argv[0], HWCLOUD_COMMAND_CALL_FUNCTION ) == 0 );
	int requestID;
	get_int( argv[1], & requestID );
	dprintf( D_PERF_TRACE, "request #%d (%s): work begins\n",
		requestID, argv[0] );
	...
	// Fill in required attributes.
	HwcloudCallFunction request = HwcloudCallFunction( requestID, argv[0] );
	request.accessKeyFile = argv[3];
	request.secretKeyFile = argv[4];
...
HwcloudBulkQuery::~HwcloudBulkQuery() { }
...
bool HwcloudBulkQuery::SendRequest() {
	bool result = HwcloudRequest::SendRequest();
	if( result ) {
...
bool HwcloudBulkQuery::workerFunction( char ** argv, int argc, std::string & result_string ) {
	...
	// Fill in required attributes.
	HwcloudBulkQuery request = HwcloudBulkQuery( requestID, argv[0] );
	request.serviceURL = argv[2];
	request.accessKeyFile = argv[3];
	request.secretKeyFile = argv[4];
